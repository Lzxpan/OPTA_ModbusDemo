...
- `192.168.2.100` 是 **目標 Opta 裝置 IP**，不是 WinForms 本機監聽 IP。
- WinForms TCP 指令通道改為監聽本機 `0.0.0.0:5000`（所有本機網卡）。
- 若某裝置無法連線，畫面狀態會顯示紅色且該裝置命令會回應 `DISCONNECTED`。

## 1. 系統架構概念

- UI 內建 **TCP 文字命令伺服器**（`0.0.0.0:5000`，即本機所有網卡）。
- 同一台電腦可用 `nc 127.0.0.1 5000`；區網其他電腦請改用「執行 WinForms 那台電腦的實際 IP」。

- Opta Controller（目標裝置）：`192.168.2.100`
- 裝置：
  - AI4：`192.168.2.111`
  - DO8：`192.168.2.112`
  - DIO4：`192.168.2.113`
