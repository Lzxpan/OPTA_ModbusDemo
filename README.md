...
- 對 Opta 的每筆讀寫在「收到 `<br>`/`<br/>` 封包結束符後」再等待 **100ms**，才允許送出下一筆命令（輪詢與插入指令共用同一節流規則）。
...
- 輪詢改為只針對「目前分頁」對應裝置執行（AI4/DO8/DIO4/DI8）。
- 輪詢背景檢查週期為 100ms（實際送出節奏仍受 I/O 閘門控制）。
- 當前分頁內會依序輪詢該裝置各 CH 的數值/狀態。
...
- 每一筆讀寫在收到 `<br>`/`<br/>` 後需等待 100ms 才可送下一筆，不會同時併發執行。
- `HELP` 會直接轉送給 Opta，並可顯示多行回應內容。
- 與 Opta 採「持續連線」模式，可用 `連線 OPTA` 按鈕手動重連。
- 偵測到 I/O 斷線時會立即停止當輪輪詢，嘗試最多 3 次自動重連；成功後會從失敗命令續跑。
